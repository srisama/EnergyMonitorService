var app = angular.module('UserPreferenceModule', ['angularUtils.directives.dirPagination']);

app.service('UserPreferenceService', function ($http) {
    /* alert("after service");*/
    ...
    this.GetAllUserPreferencebyUserId = function (userpre) {
        return $http.post(baseUrl + "/UserPreferences", userpre, { headers: { 'Content-Type': 'application / json, text / plain, */*' } });
    }

    this.GetUserPreferenceebyPreferenceid = function (userpre) {
        return $http.post(baseUrl + "/UserPreferencebyId", userpre, { headers: { 'Content-Type': 'application / json, text / plain, */*' } });
    }

    this.insertUserPreference = function (userpre) {
        return $http.post(baseUrl + "/AddUserPreference", userpre, { headers: { 'Content-Type': 'application / json, text / plain, */*' } });
    }
    this.updateUserPreference = function (userpre) {
        return $http.put(baseUrl + "/UpdateUserPreference", userpre, { headers: { 'Content-Type': 'application / json, text / plain, */*' } });
    }

    this.deleteUserPreference = function (userpre) {
        return $http.post(baseUrl + "/DeleteUserPreference", userpre, { headers: { 'Content-Type': 'application / json, text / plain, */*' } });
        //    return $http.delete(longurl + deviceid);
    }
});
app.controller('OtherController', function ($rootScope, $scope, $http, UserPreferenceService) {

});

app.controller('UserPreferenceController', function UserPreferenceController($scope, $rootScope, $http, UserPreferenceService) {

    getDevicesdropdown();
    $scope.staticuserid = 3;
    var details = { User_Id: $scope.staticuserid };
    ...
    $scope.canceluserpreference = function () {
        $('#sucessdiv').addClass('hide').html('');
        $('#errordiv').addClass('hide').html('');
        $('.sucessdiv').addClass('hide');
        $('.errordiv').addClass('hide');
    ...
    function loadAllUserPreferences(user) {
    ...
    $scope.insertuserpreference = function () {
    ...
    $scope.edituserpreference = function (preferenceid) {
        ...
        promise.then(
            ...
            function (response) {
                ...
                $('#energyusagegoal').val($scope.singleuserpreference.energy_usage_goal);
                $('#energyalertthreshold').val($scope.singleuserpreference.energy_alert_threshold);
                $('#reportfrequency').val($scope.singleuserpreference.report_frequency);
    ...
    $scope.deleteuserpreference = function (preferenceid) {
        var id = preferenceid;
